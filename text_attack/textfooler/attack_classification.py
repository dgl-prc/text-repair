...
import os
import numpy as np
import text_attack.textfooler.dataloader as dataloader
# from text_attack.textfooler.train_classifier import train_classifier
...
from text_attack.textfooler.BERT.tokenization import BertTokenizer
from text_attack.textfooler.BERT.modeling import BertForSequenceClassification, BertConfig

from models.adapter import make_classifier,PredictorTextFooler
from utils.constant import ModelType, DataCorpus, PROJECT_ROOT, SENTENCE_ENCODER
...
class USE(object):
    def __init__(self, cache_path):
        ...
        module_url = os.path.join(PROJECT_ROOT, SENTENCE_ENCODER)
        ...
        for idx, synonyms in synonyms_all:
            ...
            if np.sum(new_probs_mask) > 0:
                ...
                if new_label_prob_min < orig_prob:
                    ...
                    num_changed += 1
            text_cache = text_prime[:]
        return text_prime, num_changed, orig_label, torch.argmax(predictor([text_prime])), num_queries
...
def main():
    ...
    model = make_classifier(ModelType.TEXT_CNN, DataCorpus.NA)
    predictor = PredictorTextFooler(model)
    print("Model built!")
